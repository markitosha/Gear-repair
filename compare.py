	...
	for idx in range(nObjects):
		while True:
			...
			if (iObj in resultIdxArray) and (iGt in gtIdxArray):
				...
				break
		isGood = isGood and dist < threshold
		found = in_resultArray[iObj]
		gt = in_gtArray[iGt]
		isGood = isGood and (found['type'] == gt['type'])
		if (isGood and (gt['type'] == 'Gear')):
			isGood = isGood and (abs(found['maxR'] - gt['maxR']) < 2)
			isGood = isGood and (abs(found['minR'] - gt['minR']) < 2)
			isGood = isGood and (found['isBroken'] == gt['isBroken'])
